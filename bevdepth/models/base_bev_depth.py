...
from bevdepth.layers.backbones.base_lss_fpn import (
    BaseLSSFPN, MaskLSSFPN, MaskLSSFPNPred, MaskLSSFPNPredHeight
)
...
class MaskHeightBEVDepthPred(BaseBEVDepth):
    # TODO: Reduce grid_conf and data_aug_conf
    def __init__(self, backbone_conf, head_conf, is_train_depth=False):
        super(BaseBEVDepth, self).__init__()
        self.backbone = MaskLSSFPNPred(**backbone_conf)
        self.head = BEVDepthHead(**head_conf)
        self.is_train_depth = is_train_depth
    ...
    def forward(
        ...
        """
        if self.is_train_depth and self.training:
            x, depth_pred, mask_pred = self.backbone(x,
                                                     mats_dict,
                                                     timestamps,
                                                     is_return_depth=True)
            preds = self.head(x)
            return preds, depth_pred, mask_pred
        else:
            x, depth_pred, mask_pred = self.backbone(x, mats_dict, timestamps, is_return_depth=True)
            preds = self.head(x)
            return preds, mask_pred


class MaskHeightBEVDepthPredHeight(BaseBEVDepth):
    # TODO: Reduce grid_conf and data_aug_conf
    def __init__(self, backbone_conf, head_conf, is_train_depth=False):
        super(BaseBEVDepth, self).__init__()
        self.backbone = MaskLSSFPNPredHeight(**backbone_conf)
        self.head = BEVDepthHead(**head_conf)
        self.is_train_depth = is_train_depth
